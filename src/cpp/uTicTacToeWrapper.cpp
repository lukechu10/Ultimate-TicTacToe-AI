...
Napi::Object uTicTacToe::Init(Napi::Env env, Napi::Object exports) {
    ...
    // hook methods
    Napi::Function func = DefineClass(env, "uTictacToe", {InstanceMethod("toArray", &uTicTacToe::toArray), InstanceMethod("getValue", &uTicTacToe::getValue), InstanceMethod("availibleMoves", &uTicTacToe::availibleMoves), InstanceMethod("applyMove", &uTicTacToe::applyMove), InstanceMethod("checkSubWin", &uTicTacToe::checkSubWin), InstanceMethod("subWins", &uTicTacToe::subWins), InstanceMethod("checkForWinGlobal", &uTicTacToe::checkForWinGlobal), InstanceMethod("bestMove", &uTicTacToe::bestMove), InstanceMethod("evaluate", &uTicTacToe::evaluate)});
...
uTicTacToe::uTicTacToe(const Napi::CallbackInfo &info) : Napi::ObjectWrap<uTicTacToe>(info) {
        ...
        for (unsigned col = 0; col < 3; col++) {
            ...
            for (unsigned subRow = 0; subRow < 3; subRow++) {
                ...
                for (unsigned subCol = 0; subCol < 3; subCol++) {
                    // assign data
                    subRowData.Set(subCol, Napi::String::New(env,
                                                             squareToString(data[row][col][subRow][subCol])));
                }
            }
        }
    }
...
Napi::Value uTicTacToe::getValue(const Napi::CallbackInfo &info) {
    ...
    return Napi::String::New(env, squareToString(game.toArray()[row][col][subRow][subCol]));
}
...
Napi::Value uTicTacToe::applyMove(const Napi::CallbackInfo &info) {
    ...
    // get move
    Move mv(info[0].As<Napi::Number>(), info[1].As<Napi::Number>(), info[2].As<Napi::Number>(), info[3].As<Napi::Number>(), who);
