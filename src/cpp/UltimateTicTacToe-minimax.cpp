...
SubGameBoard<int> UltimateTicTacToe::generateCoefs() {
    ...
    for (unsigned row = 0; row < 3; row++) {
        int max = (int)count(subWins[row].begin(), subWins[row].end(), Square::Player1);
        int min = (int)count(subWins[row].begin(), subWins[row].end(), Square::Player2);
    ...
    // evaluate cols
    for (unsigned col = 0; col < 3; col++) {
        int max = (int)countNum(subWins[0][col], subWins[1][col], subWins[2][col], Square::Player1);
        int min = (int)countNum(subWins[0][col], subWins[1][col], subWins[2][col], Square::Player2);
        if ((max != 0 && min != 0) || (max == 0 && min == 0)) {
            res[0][col] += 1;
            res[1][col] += 1;
            res[2][col] += 1;
    ...
    // evaluate diaganol
    {
        int max = (int)countNum(subWins[0][0], subWins[1][1], subWins[2][2], Square::Player1);
