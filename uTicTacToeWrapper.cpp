...
Napi::Object uTicTacToe::Init(Napi::Env env, Napi::Object exports) {
	...
	// hook methods
	Napi::Function func = DefineClass(env, "uTictacToe", {
...
Napi::Value uTicTacToe::getValue(const Napi::CallbackInfo& info) {
	Napi::Env env = info.Env();
	// check arguments
	if (info.Length() != 4) throw Napi::Error::New(env, "Invalid argument length");
...
Napi::Value uTicTacToe::availibleMoves(const Napi::CallbackInfo& info) {
	...
	if (info.Length() != 1) throw Napi::Error::New(env, "Invalid argument length");
